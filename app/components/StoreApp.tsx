...
import { ArrowRight, Banknote, Check, ChevronDown, CircleAlert, Copy, Eye, EyeOff, Filter, Grape, Heart, History, KeyRound, LoaderCircle, LogOut, Mail, MapPin, Menu, Minus, Plus, QrCode, Search, ShoppingBag, Star, Trash2, UserRound, Wine, X } from "lucide-react";
...
export default function StoreApp() {
  ...
  const [accountAfterLogin, setAccountAfterLogin] = useState(() => typeof window !== "undefined" && new URLSearchParams(window.location.search).get("conta") === "pedidos");
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      try {
        setFavoriteIds(JSON.parse(localStorage.getItem("oli-vinhos-favorites") ?? "[]"));
      } catch {
        setFavoriteIds([]);
      }
    }, 0);
  ...
  const visible = useMemo(() => {
    ...
    const matches = products.filter((p) => {
      ...
      return p.active && (!q || searchable.includes(q)) && (!filters.country || p.country === filters.country) && (!filters.type || p.type === filters.type) && (!filters.grape || p.grape === filters.grape) && (!filters.producer || p.producer === filters.producer) && (!filters.vintage || String(p.vintage) === filters.vintage) && currentPrice(p) <= filters.maxPrice && (!filters.featured || p.featured) && (!filters.promotion || p.promotional_price !== null) && (!filters.available || p.quantity_available === null || p.quantity_available > 0);
    });
    ...
    setCart((items) => {
      ...
      if (!found)
        return [
          ...
          {
            product,
            quantity: Math.min(quantity, product.quantity_available ?? quantity),
          },
        ];
      return items.map((item) =>
        item.product.id === product.id
          ? {
              ...item,
              quantity: Math.min(item.quantity + quantity, product.quantity_available ?? 99),
            }
          : item,
      );
    });
    ...
    setCart((items) =>
      items.flatMap((item) => {
        ...
        return [
          {
            ...item,
            quantity: Math.min(quantity, item.product.quantity_available ?? 99),
          },
        ];
      }),
    );
  ...
  async function loadAccount(): Promise<ActionFeedback> {
    if (!supabase || !session)
      return {
        type: "error",
        text: "Sua sessão não está disponível. Entre novamente.",
      };
    setAccountLoading(true);
    const [profileResult, ordersResult, reviewsResult] = await Promise.all([supabase.from("profiles").select("*").eq("id", session.user.id).single(), supabase.from("orders").select("*, order_items(*), order_status_history(*), payment_status_history(*)").order("created_at", { ascending: false }), supabase.from("product_reviews").select("*").eq("user_id", session.user.id).order("created_at", { ascending: false })]);
    ...
    if (profileResult.error || ordersResult.error || reviewsResult.error)
      return {
        type: "error",
        text: "Não foi possível atualizar todos os dados da sua conta.",
      };
  ...
  async function submitReview(orderItemId: string, rating: number, comment: string): Promise<ActionFeedback> {
    ...
    const moderationEmail = submitted?.id
      ? await supabase.functions.invoke("send-review-moderation-email", {
          body: { reviewId: submitted.id },
        })
      : null;
    await loadAccount();
    if (!moderationEmail || moderationEmail.error || !moderationEmail.data?.sent) {
      return {
        type: "error",
        text: "A avaliação foi salva, mas o e-mail de moderação não pôde ser enviado. Ela continua disponível no painel administrativo.",
      };
    }
    return {
      type: "ok",
      text: "Avaliação enviada para moderação. A equipe recebeu os botões para aprovar ou rejeitar por e-mail.",
    };
  }
  ...
  async function sendOrder(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (error || !created) {
      ...
      setFormError(error?.message ?? "Não foi possível registrar o pedido.");
      return;
    }
  ...
  return (
    <div className="site-shell">
      <div className="announcement">
        <span>Curadoria independente • retirada local</span>
        <strong>Preços e estoque dependem de confirmação</strong>
      </div>
      <header className="header">
        <a className="brand" href="#top" aria-label="OLI Vinhos - início">
          <span>OLI</span>
          <small>VINHOS</small>
        </a>
        ...
        <div className="header-actions">
          ...
          <button className="account-button" onClick={() => (session ? setAccountOpen(true) : setAuthOpen(true))}>
            <UserRound size={18} />
            <span>{session ? "Minha conta" : "Entrar"}</span>
          </button>
      ...
      <main id="top">
        <section className="hero">
          <div className="hero-copy">
            ...
            <div className="hero-cta">
              ...
              <span>
                <Check size={16} /> Pedido salvo e acompanhado online
              </span>
            </div>
          </div>
        ...
        <section className="promise-strip" aria-label="Diferenciais">
          <div>
            ...
            <span>
              <b>16 rótulos</b>
              <small>Extraídos do catálogo OLI</small>
            </span>
          </div>
          <div>
            <Grape />
            <span>
              <b>4 países</b>
              <small>Origens para descobrir</small>
            </span>
          </div>
          <div>
            <MapPin />
            <span>
              <b>Retirada local</b>
              <small>Após confirmação da loja</small>
            </span>
          </div>
        </section>
        ...
        <section className="catalog-section" id="catalogo">
          <div className="section-heading">
            <div>
              ...
              <h2>Escolha pelo seu momento</h2>
            </div>
            <p>Explore por estilo, origem ou uva. Todos os valores seguem exatamente o catálogo fornecido.</p>
          </div>
          <div className="category-row">
            {options.types.map((type) => (
              ...
              >
                <span>{type === "Tinto" ? "●" : type === "Branco" ? "○" : "◐"}</span>
                {type}
              </button>
            ))}
            <button className={filters.promotion ? "active" : ""} onClick={() => setFilters((f) => ({ ...f, promotion: !f.promotion }))}>
              <span>%</span>Ofertas
            </button>
          </div>
          {dataMessage && (
          ...
          <div className="catalog-toolbar">
            <label className="search-box">
              <Search size={19} />
              <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Busque por vinho, uva, país..." aria-label="Buscar vinhos" />
              {query && (
          ...
          <div className="catalog-layout">
            ...
            <div className="products-area">
              <div className="results-line">
                <span>
                  {visible.length} {visible.length === 1 ? "vinho encontrado" : "vinhos encontrados"}
                </span>
                <button
                  onClick={() => {
              ...
              {loading ? (
                ...
                <div className="empty-state">
                  ...
                  <p>Tente ampliar a faixa de preço ou remover algum filtro.</p>
                  <button
                ...
                <div className="product-grid">
                  {visible.map((p) => (
                    <ProductCard
                      key={p.id}
                      product={p}
                      reviews={reviews.filter((review) => review.product_id === p.id)}
        ...
        <section className="pickup-section" id="como-funciona">
          <div className="pickup-intro">
            ...
            <p>Pedidos disponíveis somente para retirada no local.</p>
          </div>
          <div className="steps">
            <div>
              <b>01</b>
              <h3>Escolha</h3>
              <p>Explore o catálogo e adicione seus rótulos ao carrinho.</p>
            </div>
            <div>
              <b>02</b>
              <h3>Registre</h3>
              <p>Entre na sua conta, escolha a retirada e envie o resumo pelo WhatsApp.</p>
            </div>
            <div>
              <b>03</b>
              <h3>Acompanhe</h3>
              <p>Consulte o histórico e o andamento do pedido em Minha conta.</p>
            </div>
          </div>
          <div className="pickup-alert">
            <CircleAlert size={21} />
            <p>
              <strong>Atenção</strong> Preços, estoque e horário de retirada dependem de confirmação pelo WhatsApp. Não realizamos entrega.
            </p>
          </div>
        </section>
        ...
        <section className="contact-section" id="contato">
          <div>
            <p className="eyebrow">Fale com a OLI</p>
            <h2>
              Uma boa escolha
              <br />
              começa com conversa.
          ...
          <div className="contact-links">
            <a href={`https://wa.me/${STORE_CONFIG.whatsappInternational}`} target="_blank" rel="noreferrer">
              <span>
                <strong>WhatsApp</strong>
                <small>
                  {STORE_CONFIG.contactName}: {STORE_CONFIG.whatsappDisplay}
                </small>
              </span>
              <ArrowRight />
            </a>
            <a href={`mailto:${STORE_CONFIG.email}`}>
              <span>
                <strong>E-mail</strong>
                <small>{STORE_CONFIG.email}</small>
              </span>
              <ArrowRight />
            </a>
          </div>
      ...
      <footer>
        ...
        <p>
          Curadoria para bons encontros.
          <br />
          Pedidos somente para retirada.
        </p>
        <div>
          <a href="#catalogo">Catálogo</a>
          <a href="#como-funciona">Retirada</a>
          <a href={sitePath("/admin/")}>Área administrativa</a>
        </div>
        <div>
          <a href={`mailto:${STORE_CONFIG.email}`}>
            <Mail size={15} />
          ...
          <a href={`https://wa.me/${STORE_CONFIG.whatsappInternational}`}>{STORE_CONFIG.whatsappDisplay}</a>
        </div>
        <small className="copyright">© 2026 OLI Vinhos</small>
      </footer>
      <a className="whatsapp-float" href={`https://wa.me/${STORE_CONFIG.whatsappInternational}`} target="_blank" rel="noreferrer" aria-label="Falar com a OLI Vinhos pelo WhatsApp">
        WA
      </a>

      {filterOpen && (
        <div className="drawer-overlay" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && setFilterOpen(false)}>
          <aside className="mobile-filter drawer">
            <div className="drawer-head">
              <h2>Filtros</h2>
              <button onClick={() => setFilterOpen(false)} aria-label="Fechar filtros">
                <X />
              </button>
            </div>
            <FilterPanel options={options} filters={filters} setFilters={setFilters} mobile />
            <button className="primary-button wide" onClick={() => setFilterOpen(false)}>
              Ver {visible.length} vinhos
            </button>
          </aside>
        </div>
      )}
      {menuOpen && (
        <div className="drawer-overlay" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && setMenuOpen(false)}>
          <aside className="mobile-nav drawer">
            ...
            <nav>
              ...
              >
                <UserRound /> {session ? "Minha conta" : "Entrar ou cadastrar"}
              </button>
            </nav>
          </aside>
        </div>
      ...
      {detail && (
        <ProductModal
          product={detail}
          reviews={reviews.filter((review) => review.product_id === detail.id)}
          quantity={detailQty}
          setQuantity={setDetailQty}
    ...
    <article className="product-card">
      <div className="product-media">
        <div className="badges">
          {product.promotional_price !== null && <span className="sale">OFERTA</span>}
          {product.low_stock && <span>POUCAS UNIDADES</span>}
          {product.featured && <span>DESTAQUE</span>}
        </div>
        <button className={`heart ${favorite ? "active" : ""}`} onClick={onFavorite} aria-pressed={favorite} aria-label={`${favorite ? "Remover" : "Adicionar"} ${product.name} ${favorite ? "dos" : "aos"} favoritos`}>
          <Heart size={18} />
        </button>
        <img src={assetUrl(product.image_url ?? "/products/placeholder.webp")} alt={`Garrafa do vinho ${product.name}`} loading="lazy" />
      </div>
      <div className="product-body">
        <p className="product-origin">{[product.country, product.region].filter(Boolean).join(" • ")}</p>
        <h3>{product.name}</h3>
        <p className="product-meta">{[product.type, product.grape, product.vintage].filter(Boolean).join(" · ")}</p>
        {reviews.length > 0 && (
        ...
        <div className="price-row">
          <div>
            {product.promotional_price !== null && <del>{money.format(product.normal_price)}</del>}
            <strong>{money.format(currentPrice(product))}</strong>
          </div>
          <span className={unavailable ? "unavailable" : "available"}>{unavailable ? "Indisponível" : product.quantity_available === null ? "Sob confirmação" : "Disponível"}</span>
        </div>
        <div className="card-actions">
          <button onClick={onDetail} className="detail-button">
            Ver detalhes
          </button>
          <button onClick={onAdd} disabled={unavailable} className="add-button" aria-label={`Adicionar ${product.name} ao carrinho`}>
            <Plus /> Adicionar
          </button>
        </div>
      </div>
...
function FilterPanel({
  ...
  const select = (label: string, key: keyof Filters, values: (string | number)[]) =>
    values.length ? (
      <label className="filter-field">
        <span>{label}</span>
        <select value={String(filters[key])} onChange={(e) => setFilters((f) => ({ ...f, [key]: e.target.value }))}>
  ...
  return (
    <aside className={`filter-panel ${mobile ? "inside-drawer" : "desktop-filter"}`}>
      <div className="filter-title">
        <h3>Filtrar por</h3>
        <button onClick={() => setFilters(initialFilters)}>Limpar</button>
      </div>
      {select("País", "country", options.countries)}
      {select("Tipo", "type", options.types)}
      {select("Uva", "grape", options.grapes)}
      {select("Produtor", "producer", options.producers)}
      {select("Safra", "vintage", options.vintages)}
      <label className="price-filter">
        <span>
          Até <b>{money.format(filters.maxPrice)}</b>
        </span>
        <input type="range" min="30" max="150" step="10" value={filters.maxPrice} onChange={(e) => setFilters((f) => ({ ...f, maxPrice: Number(e.target.value) }))} />
        <div>
          <small>R$ 30</small>
          <small>R$ 150</small>
        </div>
      ...
      <div className="check-filters">
        ...
        <label>
          <input type="checkbox" checked={filters.promotion} onChange={(e) => setFilters((f) => ({ ...f, promotion: e.target.checked }))} />
          <span />
          Em promoção
        </label>
        <label>
...
function CartDrawer({ cart, total, onClose, onQuantity, onClear, onCheckout }: { cart: CartItem[]; total: number; onClose: () => void; onQuantity: (id: string, q: number) => void; onClear: () => void; onCheckout: () => void }) {
  return (
    <div className="drawer-overlay" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <aside className="cart-drawer drawer" aria-label="Carrinho">
        <div className="drawer-head">
          <div>
            <p>SEU PEDIDO</p>
            <h2>Carrinho</h2>
        ...
        {cart.length === 0 ? (
          <div className="empty-cart">
            ...
            <p>Escolha um rótulo para começar.</p>
          ...
          <>
              ...
              {cart.map((item) => (
                <div className="cart-item" key={item.product.id}>
                  ...
                  <div>
                    ...
                    <small>
                      {item.product.volume} • {item.product.vintage}
                    </small>
                    <strong>{money.format(currentPrice(item.product))}</strong>
                    <div className="quantity">
                      <button onClick={() => onQuantity(item.product.id, item.quantity - 1)} aria-label="Diminuir quantidade">
                        {item.quantity === 1 ? <Trash2 size={15} /> : <Minus size={15} />}
                      </button>
                      <span>{item.quantity}</span>
                      <button onClick={() => onQuantity(item.product.id, item.quantity + 1)} aria-label="Aumentar quantidade">
                        <Plus size={15} />
                      </button>
                    </div>
                  </div>
                  <b>{money.format(currentPrice(item.product) * item.quantity)}</b>
                </div>
              ))}
            </div>
            ...
            <div className="cart-summary">
              ...
              <small>
                <CircleAlert size={15} /> Este pedido será preparado somente para retirada no local.
              </small>
              <button className="primary-button wide" onClick={onCheckout}>
                Continuar pedido <ArrowRight size={18} />
              </button>
              <p className="confirmation-note">Estoque e horário sujeitos à confirmação.</p>
            </div>
          </>
        )}
      </aside>
...
function ProductModal({ product, reviews, quantity, setQuantity, onClose, onAdd }: { product: WineProduct; reviews: ProductReview[]; quantity: number; setQuantity: (q: number) => void; onClose: () => void; onAdd: () => void }) {
  const fields = [
    ...
    ["Origem", [product.region, product.country].filter(Boolean).join(", ")],
    ["Tipo", product.type],
    ["Uva", product.grape],
    ["Composição", product.grape_composition],
    ["Safra", product.vintage],
    ["Volume", product.volume],
    ["Teor alcoólico", product.alcohol_content],
    ["Classificação", product.classification],
    ["Serviço", product.service_temperature],
  ].filter(([, v]) => v);
  return (
    <div className="modal-overlay" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <div className="product-modal">
        ...
        <div className="modal-media">
          <img src={assetUrl(product.image_url)} alt={`Garrafa de ${product.name}`} />
        </div>
        <div className="modal-content">
          ...
          <div className="modal-buy">
            <div className="quantity">
              ...
              <span>{quantity}</span>
              <button onClick={() => setQuantity(Math.min(product.quantity_available ?? 99, quantity + 1))}>
                <Plus />
              </button>
            </div>
            <button className="primary-button" onClick={onAdd} disabled={product.quantity_available === 0}>
              Adicionar • {money.format(currentPrice(product) * quantity)}
            </button>
          </div>
          <small className="stock-copy">{product.quantity_available === null ? "Disponibilidade confirmada pela loja no WhatsApp." : `${product.quantity_available} unidade(s) disponível(is).`}</small>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="modal-overlay">
      <div className="checkout-modal">
        ...
        <h2>Pagamento e retirada</h2>
        <p>Não realizamos entrega. Escolha a forma de pagamento e o horário desejado para retirada.</p>
        <form onSubmit={onSubmit}>
          ...
          <div className="form-row">
            ...
            <label>
              Horário aproximado *<input name="time" type="time" required />
            </label>
          </div>
          <fieldset className="payment-options">
            ...
            <label>
              ...
              <span>
                <QrCode />
                <b>Pix</b>
                <small>{isProduction ? "Copie a chave Pix após registrar o pedido. A confirmação é manual." : "Na homologação, nenhum valor real será movimentado."}</small>
              </span>
            </label>
            <label>
              ...
              <span>
                <Banknote />
                <b>Dinheiro na retirada</b>
                <small>Pague ao receber o pedido no local.</small>
              </span>
            </label>
          </fieldset>
          <label>
          ...
          <div className="order-preview">
            <span>{cart.reduce((s, i) => s + i.quantity, 0)} item(ns)</span>
            <strong>{money.format(total)}</strong>
          </div>
          <button className="primary-button wide" type="submit" disabled={submitting}>
            {submitting ? (
              <>
                <LoaderCircle className="spin" /> Registrando pedido…
              </>
            ) : (
              <>
                Registrar e enviar pedido <ArrowRight size={18} />
              </>
            )}
          </button>
          <small>Pedido, estoque e horário dependem da confirmação da loja. Retirada somente no local.</small>
        </form>
      </div>
    </div>
  );
...
function CustomerAuthModal({ onClose }: { onClose: () => void }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!supabase) {
      setFeedback({
        type: "error",
        text: "A conexão com a área de clientes não está disponível. Atualize a página e tente novamente.",
      });
      return;
    }
    const data = new FormData(event.currentTarget);
    ...
    if (mode === "reset") {
      ...
      if (error) setFeedback({ type: "error", text: "E-mail ou senha inválidos." });
      else onClose();
    } else {
      ...
      const { data: result, error } = await supabase.auth.signUp({
        ...
        password,
        options: {
          emailRedirectTo: `${window.location.origin}${sitePath("/")}`,
          data: {
      ...
      else if (result.session) onClose();
      else
        setFeedback({
          type: "ok",
          text: "Cadastro recebido. Confira seu e-mail para confirmar a conta e depois faça login.",
        });
    }
    setBusy(false);
  }
  return (
    <div className="modal-overlay">
      <div className="auth-modal">
        <button type="button" className="modal-close" onClick={onClose} aria-label="Fechar" disabled={busy}>
          <X />
        </button>
        ...
        <form onSubmit={submit}>
          ...
          {mode === "signup" && (
            <label>
              Confirmar senha
              <CustomerPasswordInput name="confirmation" required minLength={8} autoComplete="new-password" disabled={busy} />
            </label>
          )}
...
function ActionMessage({ feedback }: { feedback: ActionFeedback }) {
  return (
    <div className={`form-feedback ${feedback.type}`} role="status">
      {feedback.type === "ok" ? <Check size={17} /> : <CircleAlert size={17} />}
...
function CustomerAccountModal({ profile, orders, reviews, loading, onRefresh, onSaveProfile, onSubmitReview, onClose, onSignOut }: { profile: CustomerProfile | null; orders: CustomerOrder[]; reviews: ProductReview[]; loading: boolean; onRefresh: () => Promise<ActionFeedback>; onSaveProfile: (e: React.FormEvent<HTMLFormElement>) => Promise<ActionFeedback>; onSubmitReview: (productId: string, rating: number, comment: string) => Promise<ActionFeedback>; onClose: () => void; onSignOut: () => Promise<void> | void }) {
  ...
  async function copyPix(order: CustomerOrder) {
    ...
    try {
      ...
      setRefreshFeedback({
        type: "error",
        text: "Não foi possível copiar automaticamente. Selecione o código e copie manualmente.",
      });
    }
  }
  ...
  async function changePassword(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!supabase) {
      setPasswordFeedback({
        type: "error",
        text: "A conexão não está disponível. Atualize a página e tente novamente.",
      });
      return;
    }
    ...
    if (error)
      setPasswordFeedback({
        type: "error",
        text: `Não foi possível alterar a senha: ${error.message}`,
      });
    else {
      form.reset();
      setPasswordFeedback({ type: "ok", text: "Senha alterada com sucesso." });
    }
  }
  return (
    <>
      <div className="drawer-overlay">
        <aside className="account-drawer drawer">
          <div className="drawer-head">
            <div>
              <p>ÁREA DO CLIENTE</p>
              <h2>Minha conta</h2>
          ...
          {tab === "orders" ? (
            <div className="account-content">
              <div className="account-title">
                ...
                <button type="button" onClick={refreshAccount} disabled={loading} aria-label="Atualizar pedidos" title="Atualizar pedidos">
                  <LoaderCircle className={loading ? "spin" : ""} />
                </button>
              </div>
              {refreshFeedback && <ActionMessage feedback={refreshFeedback} />}{" "}
              {loading ? (
                <p className="account-empty">Carregando pedidos…</p>
              ) : orders.length === 0 ? (
                <div className="account-empty">
                  <ShoppingBag />
                  <h3>Nenhum pedido ainda</h3>
                  <p>Seus próximos pedidos aparecerão aqui.</p>
                </div>
              ) : (
                orders.map((order) => (
                  <article className="customer-order" key={order.id}>
                    ...
                    <p>
                      {dateTime.format(new Date(order.created_at))} • Retirada {new Date(`${order.pickup_date}T12:00:00`).toLocaleDateString("pt-BR")} às {order.pickup_time.slice(0, 5)}
                    </p>
                    <div className="customer-order-items">
                      {order.order_items.map((item) => {
                        const existing = reviews.find((review) => review.order_item_id === item.id);
                        return (
                          <div key={item.id}>
                            <span>
                              {item.quantity}× {item.product_name}
                            </span>
                            <b>{money.format(Number(item.line_total))}</b>
                            {order.status === "delivered" && item.product_id && (
                              ...
                              >
                                {existing ? `Avaliação ${existing.status === "approved" ? "publicada" : existing.status === "rejected" ? "rejeitada" : "em análise"}` : "Avaliar produto"}
                              </button>
                            )}
                          </div>
                        );
                    ...
                    <div className={`customer-payment ${order.payment_status}`}>
                      <span>{paymentMethodLabel[order.payment_method] ?? order.payment_method}</span>
                      <b>{paymentStatusLabel[order.payment_status]}</b>
                      {order.payment_method === "pix" && order.pix_copy_paste && order.payment_status === "pending" && (
                        <button type="button" onClick={() => copyPix(order)} className={copiedPixOrderId === order.id ? "copied" : ""}>
                          <Copy />
                          {copiedPixOrderId === order.id ? "Código Pix copiado!" : isProduction ? "Copiar chave Pix" : "Copiar Pix de homologação"}
                        </button>
                      )}
                    </div>
                    {copiedPixOrderId === order.id && (
                      <div className="copy-feedback" role="status">
                        <Check /> Código Pix copiado para a área de transferência.
                      </div>
                    )}
                    <div className="status-timeline">
                      {order.order_status_history.map((history) => (
                        <div key={history.id}>
                          <i />
                          <span>
                            <b>{orderStatusLabel[history.status]}</b>
                            <small>
                              {dateTime.format(new Date(history.created_at))}
            ...
            <div className="account-content">
              <form className="account-form" onSubmit={saveProfileForm}>
              ...
              <form className="account-form" onSubmit={changePassword}>
                ...
                <label>
                  Nova senha
                  <CustomerPasswordInput name="password" minLength={8} required autoComplete="new-password" disabled={passwordBusy} />
                </label>
                {passwordFeedback && <ActionMessage feedback={passwordFeedback} />}
                <button className="secondary-button" type="submit" disabled={passwordBusy}>
                  {passwordBusy ? (
                    <>
                      <LoaderCircle className="spin" /> Atualizando…
                    </>
                  ) : (
                    "Atualizar senha"
                  )}
          ...
          <div className="account-footer">
            ...
            >
              <LogOut /> {signOutBusy ? "Saindo…" : "Sair da conta"}
            </button>
          </div>
        </aside>
      </div>
      {reviewing && <ReviewModal target={reviewing} existing={reviews.find((review) => review.order_item_id === reviewing.orderItemId)} onClose={() => setReviewing(null)} onSubmit={onSubmitReview} />}
    </>
  );
}

function ReviewModal({ target, existing, onClose, onSubmit }: { target: { orderItemId: string; productId: string; productName: string }; existing?: ProductReview; onClose: () => void; onSubmit: (orderItemId: string, rating: number, comment: string) => Promise<ActionFeedback> }) {
  const [feedback, setFeedback] = useState<ActionFeedback | null>(null);
  const [busy, setBusy] = useState(false);
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    ...
    setBusy(true);
    setFeedback(null);
    const result = await onSubmit(target.orderItemId, Number(data.get("rating")), String(data.get("comment") ?? ""));
    setFeedback(result);
    setBusy(false);
    if (result.type === "ok") form.reset();
  }
  return (
    <div className="modal-overlay review-overlay">
      <form className="review-modal" onSubmit={submit}>
        ...
        <p>{target.productName}</p>
        <fieldset className="star-rating">
          <legend>Sua nota</legend>
          {[5, 4, 3, 2, 1].map((value) => (
            <label key={value}>
              <input type="radio" name="rating" value={value} required defaultChecked={existing?.rating === value} />
              <span>
        ...
        <small>A avaliação será publicada após a moderação da OLI.</small>
      </form>
    </div>
  );
}
  ...
  return (
    <div className="modal-overlay">
      <div className="order-success">
        ...
        <p>
          Recebemos seu pedido no valor de <strong>{money.format(Number(order.total))}</strong>. Ele está pendente e aguardando confirmação da OLI Vinhos.
        </p>
        <section className={`success-payment ${order.payment_method}`}>
          <div>
            {order.payment_method === "pix" ? <QrCode /> : <Banknote />}
            <span>
              <small>Forma de pagamento</small>
              <b>{paymentMethodLabel[order.payment_method]}</b>
            </span>
          </div>
          {order.payment_method === "cash" ? (
            <p>Pague em dinheiro no momento da retirada.</p>
          ) : (
            <>
              <p>
                {isProduction ? (
                  <>
                    <strong>Chave Pix:</strong> copie a chave abaixo e aguarde a confirmação manual do pagamento.
                  </>
                ) : (
                  <>
                    <strong>Pix de homologação:</strong> isto é apenas um teste. Não efetue nenhum pagamento real.
                  </>
                )}
              </p>
              {order.pix_copy_paste && (
                <button type="button" onClick={copyPix} className={pixCopied ? "copied" : ""}>
                  {pixCopied ? <Check /> : <Copy />}
                  {pixCopied ? "Código Pix copiado!" : isProduction ? "Copiar chave Pix" : "Copiar código Pix de teste"}
                </button>
              )}
              {pixCopied && (
                <div className="copy-feedback" role="status">
        ...
        <small>{emailNotice}</small>
        <div className="order-send-actions">
          <a className="primary-button" href={whatsapp} target="_blank" rel="noreferrer">
            Abrir pedido no WhatsApp
          </a>
        </div>
        <button className="account-history-link" onClick={onAccount}>
          Acompanhar em Minha conta <ArrowRight />
        </button>
      </div>
    </div>
  );
