...
import { ArrowLeft, CircleAlert, Edit3, ImageUp, KeyRound, LogOut, Package, Plus, RefreshCw, Search, Trash2, UserPlus, Wine, X } from "lucide-react";
...
export default function AdminApp() {
  ...
  async function createAdmin(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!supabase) return; const data = new FormData(event.currentTarget); const email = String(data.get("email")); const password = String(data.get("password")); const confirmation = String(data.get("confirmation"));
    ...
    if (error) setMessage({ type: "error", text: "Não foi possível criar o administrador. Verifique se o e-mail já está cadastrado." });
    else { setMessage({ type: "ok", text: `Administrador ${created.email} criado com sucesso.` }); setCreatingAdmin(false); }
  }
...
function CreateAdminForm({ onClose, onSave }: { onClose:()=>void; onSave:(e:React.FormEvent<HTMLFormElement>)=>void }) {
  return <div className="admin-form-overlay password-overlay"><form className="password-form" onSubmit={onSave}><div className="form-head"><div><small>ACESSOS</small><h2>Novo administrador</h2></div><button type="button" onClick={onClose} aria-label="Fechar"><X/></button></div><div className="password-fields"><label>E-mail da pessoa<input name="email" type="email" required autoComplete="off"/></label><label>Senha temporária<input name="password" type="password" minLength={8} required autoComplete="new-password"/></label><label>Confirmar senha<input name="confirmation" type="password" minLength={8} required autoComplete="new-password"/></label><small>A pessoa poderá entrar, gerenciar produtos e trocar a própria senha.</small></div><div className="form-footer"><button type="button" onClick={onClose}>Cancelar</button><button className="primary-button" type="submit">Criar administrador</button></div></form></div>;
}
