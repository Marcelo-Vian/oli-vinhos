...
export default function AdminApp() {
  ...
  async function updateUser(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!supabase || !editingUser || editingUserBusy || currentRole !== "master") return;
    const data = new FormData(event.currentTarget);
    const email = String(data.get("email")).trim().toLowerCase();
  ...
  const canManageAccess = currentRole === "master" || currentRole === "admin";
...
function AdminsSection({ members, currentUserId, canManage, canEdit, onEdit, onRefresh, onCreate, onReset, onRemove, onRole }: { members:CustomerProfile[]; currentUserId:string; canManage:boolean; canEdit:boolean; onEdit:(member:CustomerProfile)=>void; onRefresh:()=>void; onCreate:()=>void; onReset:(member:CustomerProfile)=>void; onRemove:(member:CustomerProfile)=>void; onRole:(member:CustomerProfile,role:"admin"|"manager")=>void }) {
  return <><header><div><p>Controle de acesso</p><h1>Equipe e acessos</h1></div><button className="primary-button" onClick={onCreate}><UserPlus/> Novo acesso</button></header><div className="admin-stats admin-access-stats"><div><span>Acessos ativos</span><b>{members.length}</b></div><div><span>MASTER</span><b>{members.filter(member=>member.role==="master").length}</b></div><div><span>Administradores gerais</span><b>{members.filter(member=>member.role==="admin").length}</b></div><div><span>Gestores da loja</span><b>{members.filter(member=>member.role==="manager").length}</b></div></div><div className="admin-toolbar admin-access-toolbar"><p>MASTER é o proprietário protegido. Administrador controla acessos. Gestor cuida da operação e pode cadastrar outros gestores.</p><button onClick={onRefresh} aria-label="Atualizar"><RefreshCw/></button></div><div className="admin-table-wrap"><table className="admin-table customers-table"><thead><tr><th>Membro da equipe</th><th>Conta criada em</th><th>Perfil de acesso</th><th>Status</th><th aria-label="Ações"/></tr></thead><tbody>{members.map((member)=>{const protectedMember=member.role==="master";return <tr key={member.id}><td><strong>{member.full_name || member.email || "Membro da equipe"}</strong><small>{member.email || "E-mail não informado"}</small></td><td>{new Date(member.created_at).toLocaleDateString("pt-BR")}</td><td><select className="staff-role-select" value={member.role} disabled={member.id===currentUserId||protectedMember||!canManage} onChange={(event)=>onRole(member,event.target.value as "admin"|"manager")}>{protectedMember&&<option value="master">MASTER — proprietário</option>}<option value="admin">Administrador geral</option><option value="manager">Gestor da loja</option></select></td><td><span className={`admin-access-badge ${protectedMember?"master":""}`}>{protectedMember?"Protegido":member.id===currentUserId?"Sua conta":"Ativo"}</span></td><td><div className="row-actions"><button onClick={()=>onEdit(member)} aria-label={`Editar ${member.email}`} title={protectedMember?"O MASTER altera os próprios dados na conta":!canEdit?"Somente o MASTER pode editar dados":"Editar usuário"} disabled={protectedMember||!canEdit}><Edit3/></button><button onClick={()=>onReset(member)} aria-label={`Alterar senha de ${member.email}`} title={protectedMember?"MASTER altera a própria senha":!canManage?"Somente MASTER ou administrador geral":"Definir senha temporária"} disabled={protectedMember||!canManage}><KeyRound/></button><button onClick={()=>onRemove(member)} aria-label={`Remover ${member.email}`} title={protectedMember?"O MASTER não pode ser removido":!canManage?"Somente MASTER ou administrador geral":member.id===currentUserId?"Você não pode remover a própria conta":"Remover acesso"} disabled={member.id===currentUserId||protectedMember||!canManage}><Trash2/></button></div></td></tr>})}</tbody></table>{members.length===0&&<p className="admin-empty">Nenhum acesso da equipe encontrado.</p>}</div></>;
}
