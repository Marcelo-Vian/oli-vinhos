...
export default function AdminApp() {
  ...
  const [currentRole, setCurrentRole] = useState<"admin" | "manager" | null>(null);
  ...
  async function verifyAdmin() {
    ...
    const role = data?.role === "admin" || data?.role === "manager" ? data.role : null;
  ...
  return <div className="admin-shell"><aside className="admin-nav"><div className="admin-logo"><Wine/> OLI <span>{currentRole==="admin"?"ADMIN":"GESTOR"}</span></div><nav><button className={section === "products" ? "active" : ""} onClick={() => setSection("products")}><Package/> Produtos</button><button className={section === "orders" ? "active" : ""} onClick={() => setSection("orders")}><ClipboardList/> Pedidos</button><button className={section === "customers" ? "active" : ""} onClick={() => setSection("customers")}><Users/> Clientes</button>{currentRole==="admin"&&<button className={section === "admins" ? "active" : ""} onClick={() => setSection("admins")}><UserPlus/> Equipe e acessos</button>}<a href={sitePath("/")} target="_blank"><Wine/> Ver loja</a></nav><div className="admin-user"><small>{currentRole==="admin"?"Administrador geral":"Gestor da loja"}</small><span>{session.user.email}</span>{currentRole==="admin"&&<button onClick={() => { setAdminCreationFeedback(null); setCreatingAdmin(true); }}><UserPlus/> Novo acesso</button>}<button onClick={() => setChangingPassword(true)}><KeyRound/> Alterar minha senha</button><button onClick={() => supabase?.auth.signOut()}><LogOut/> Sair</button></div></aside><main className="admin-main">{message && <AdminMessage message={message}/>} {section === "products" && <ProductsSection products={products} visible={visible} loading={loading} query={query} status={status} setQuery={setQuery} setStatus={setStatus} onRefresh={()=>loadProducts(true)} onNew={() => setEditing({ ...blank })} onEdit={(product) => setEditing({ ...product })} onRemove={removeProduct} onToggle={toggleProduct}/>} {section === "orders" && <OrdersSection orders={orders} filter={orderFilter} setFilter={setOrderFilter} onRefresh={()=>loadOrders(true)} onSelect={setSelectedOrder} onStatus={changeOrderStatus}/>} {section === "customers" && <CustomersSection customers={customers} orders={orders} onRefresh={async () => { await Promise.all([loadCustomers(), loadOrders()]); setMessage({type:"ok",text:"Clientes atualizados."}); }}/>} {section === "admins" && currentRole==="admin" && <AdminsSection members={customers.filter((profile)=>profile.role==="admin"||profile.role==="manager")} currentUserId={session.user.id} onRefresh={()=>loadCustomers(true)} onCreate={()=>{ setAdminCreationFeedback(null); setCreatingAdmin(true); }} onReset={resetAdminPassword} onRemove={removeAdmin} onRole={changeStaffRole}/>}</main>{editing&&<ProductForm product={editing} busy={savingProduct} setProduct={setEditing} onClose={()=>{if(!savingProduct)setEditing(null);}} onSave={saveProduct} onMessage={setMessage}/>} {changingPassword&&<PasswordForm busy={changingPasswordBusy} onClose={()=>{if(!changingPasswordBusy)setChangingPassword(false);}} onSave={changePassword}/>} {creatingAdmin&&<CreateAdminForm feedback={adminCreationFeedback} busy={creatingAdminBusy} onClose={()=>{ if (!creatingAdminBusy) { setCreatingAdmin(false); setAdminCreationFeedback(null); } }} onSave={createAdmin}/>} {selectedOrder&&<OrderDetail order={selectedOrder} onClose={()=>setSelectedOrder(null)} onStatus={async (next) => { await changeOrderStatus(selectedOrder,next); setSelectedOrder(null); }}/>}</div>;
}
...
function AdminsSection({ members, currentUserId, onRefresh, onCreate, onReset, onRemove, onRole }: { members:CustomerProfile[]; currentUserId:string; onRefresh:()=>void; onCreate:()=>void; onReset:(member:CustomerProfile)=>void; onRemove:(member:CustomerProfile)=>void; onRole:(member:CustomerProfile,role:"admin"|"manager")=>void }) {
  return <><header><div><p>Controle de acesso</p><h1>Equipe e acessos</h1></div><button className="primary-button" onClick={onCreate}><UserPlus/> Novo acesso</button></header><div className="admin-stats admin-access-stats"><div><span>Acessos ativos</span><b>{members.length}</b></div><div><span>Administradores gerais</span><b>{members.filter(member=>member.role==="admin").length}</b></div><div><span>Gestores da loja</span><b>{members.filter(member=>member.role==="manager").length}</b></div></div><div className="admin-toolbar admin-access-toolbar"><p>Administrador geral controla acessos. Gestor cuida de produtos, estoque, clientes e pedidos.</p><button onClick={onRefresh} aria-label="Atualizar"><RefreshCw/></button></div><div className="admin-table-wrap"><table className="admin-table customers-table"><thead><tr><th>Membro da equipe</th><th>Conta criada em</th><th>Perfil de acesso</th><th>Status</th><th aria-label="Ações"/></tr></thead><tbody>{members.map((member)=><tr key={member.id}><td><strong>{member.full_name || member.email || "Membro da equipe"}</strong><small>{member.email || "E-mail não informado"}</small></td><td>{new Date(member.created_at).toLocaleDateString("pt-BR")}</td><td><select className="staff-role-select" value={member.role} disabled={member.id===currentUserId} onChange={(event)=>onRole(member,event.target.value as "admin"|"manager")}><option value="admin">Administrador geral</option><option value="manager">Gestor da loja</option></select></td><td><span className="admin-access-badge">{member.id===currentUserId?"Sua conta":"Ativo"}</span></td><td><div className="row-actions"><button onClick={()=>onReset(member)} aria-label={`Alterar senha de ${member.email}`} title="Definir senha temporária"><KeyRound/></button><button onClick={()=>onRemove(member)} aria-label={`Remover ${member.email}`} title={member.id===currentUserId?"Você não pode remover a própria conta":"Remover acesso"} disabled={member.id===currentUserId}><Trash2/></button></div></td></tr>)}</tbody></table>{members.length===0&&<p className="admin-empty">Nenhum acesso da equipe encontrado.</p>}</div></>;
}
